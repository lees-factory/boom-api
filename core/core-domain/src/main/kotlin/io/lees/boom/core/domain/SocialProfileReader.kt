...
class SocialProfileReader {
    suspend fun read(provider: SocialProvider, token: String): SocialProfile {
    ...
    private fun readAppleProfile(token: String): SocialProfile {
        return SocialProfile(SocialProvider.APPLE, "sub_a", "a@test.com", "AUser")
    }

    private fun readKakaoProfile(token: String): SocialProfile {
        return SocialProfile(SocialProvider.KAKAO, "id_k", "k@test.com", "KUser")
    }
}